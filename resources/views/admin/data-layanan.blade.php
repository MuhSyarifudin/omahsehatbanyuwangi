...
<div class="bg-white rounded-lg px-8 py-6 overflow-x-scroll mb-12">
    ...
    <h4 class="text-xl font-semibold mb-5">Data Layanan Terapi</h4>
    ...
    <a id="btnTambahLayanan"
       class="inline-flex cursor-pointer hover:cursor-pointer items-center gap-2 bg-blue-600 hover:bg-opacity-90 text-white text-sm font-medium px-4 py-2 mb-4 rounded-lg shadow transition duration-200">
                ...
                <div class="w-full max-w-lg rounded-2xl bg-white shadow-xl">
                        ...
                        <form id="formTambahLayanan" class="space-y-4 px-6 py-5">
                            ...
                            <div>
                                ...
                                <select
                                    name="jenis"
                                    class="w-full rounded-lg border px-4 py-2 focus:border-blue-500 focus:ring-2 focus:ring-blue-500 text-md">
                                    <option value="">Pilih Jenis Terapi</option>
                                    @foreach ($jenisTerapi as $item)
                ...
                <form id="formEditLayanan" class="space-y-4 px-6 py-5">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            id="edit_nama"
                            name="nama"
                            class="w-full rounded-lg border px-4 py-2 focus:border-blue-600 focus:ring-1 focus:ring-blue-500"
                            required>
                    </div>
                    ...
                    <div>
                        ...
                        <select
                            id="edit_jenis_terapi"
                            name="jenis_terapi"
                            class="w-full rounded-lg border px-4 py-2 focus:border-blue-500 text-md focus:ring-2 focus:ring-blue-500"
                            required>
                            <option value="">Pilih Jenis Terapi</option>
                            @foreach ($jenisTerapi as $item)
                                <option value="{{ $item->id }}">{{ $item->nama }}</option>
        ...
        <table class="w-full my-8 whitespace-nowrap" id="tabel_layanan">
            <thead>
                <tr>
...
<script>
    $(document).ready(function () {
       let table = $('#tabel_layanan').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: "{{ route('data-layanan.datatables') }}"
        },
        columns: [
        ...
        $('#btnTambahLayanan').on('click', function () {
            $('#modalTambahLayanan').removeClass('hidden').addClass('flex');
        });
    ...
    $(document).on('click', '.btnEditLayanan', function () {
        ...
        $('#edit_jenis_terapi').val($(this).data('jenis'));
        $('#edit_harga').val($(this).data('harga'));

        $('#modalEditLayanan').removeClass('hidden').addClass('flex');
    });

    // CLOSE MODAL
    $(document).on('click', '.btnCloseEdit', function () {
        $('#modalEditLayanan').addClass('hidden').removeClass('flex');
    });
    ...
    $('#formEditLayanan').on('submit', function (e) {
        ...
        $.ajax({
            url: '{{ route("data.layanan.update",["id"=>"__id__"]) }}'.replace('__id__',id),
            type: 'PUT',
            data: $(this).serialize(),
            success: function (res) {
                $('#modalEditLayanan').addClass('hidden').removeClass('flex');
            ...
            error: function (xhr) {
                alert(xhr.responseJSON?.message ?? 'Gagal update data');
            }
        });
    });
    ...
    $('#btnConfirmDelete').on('click', function () {
        ...
        $.ajax({
            url: '{{ route("data.layanan.destroy",["id"=>"__id__"]) }}'.replace('__id__',deleteId),
            type: 'DELETE',
            success: function (res) {
                $('#modalDeleteLayanan')
                    .addClass('hidden')
